...
| System Architecture Explorer | Promoted from an ad-hoc design-record page. Editable component-price BOM with source/confidence tags, live DKK↔EUR converter, sortable/filterable 19-variant control-electronics matrix (driver type, comms layer, complexity, cost, pins free) with expandable per-variant BOM breakdowns, a pin-budget feasibility readout (selectable SPI vs 8-bit-parallel screen scenario), and a live variant-driven SVG system diagram. Persists prices/rate via localStorage. Partially retires the "Bill of materials / component selector" backlog item. |
