...
- **D-04:** **Do not design the bottom layer in this phase.** The user will supply the actual prototype testing protocol. Phase 9 planning designs and (if executed) builds the **top layer** first; the bottom layer's structure (likely an ISO-requirement | prototype-reality | justification deviation table, seeded from proto-02's already-documented deviations) is deferred until the protocol is provided. Planning must leave a clean, obvious slot for it.
...
### The prototype's actual method + data (feeds the bottom layer, deferred per D-04)
