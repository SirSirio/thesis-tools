...
- Phase 10 added (2026-07-31): Prototype Design Space — Alignment Module thread + detail subpage split. Second parallel journey thread in a violet/indigo accent family, journey stage/JS de-singularized for N threads, monolithic detail views extracted into per-prototype subpages, new align-01 subpage for the V2.1 homing-microswitch build.
...
| 2026-09-01 | ui-prototypes: publish the operator-interface design record as a tool page (20260901-ui-prototypes): new `tools/ui-prototypes/` — the five design-record HTML artifacts (rounds 1–3 designing the interface 2026-07-31→08-09, rounds 4–5 home-screen colour/structure studies 2026-08-11) copied **verbatim** into `rounds/`, wrapped by a site-pattern index page (two-arc framing, self-audit instrument explained, live scaled inert-iframe previews of the artifacts themselves, provenance panel). Exactly two mechanical edits per artifact set: Google Fonts links in the two home studies repointed to vendored Montserrat (`assets/fonts/montserrat/`, v31 variable woff2 latin+latin-ext, OFL 1.1) and one "← All rounds" back-link inserted per file. Landing card (EN+IT), README/ROADMAP/CLAUDE.md rows. **Path frozen: thesis ch. 9 prints `sirsirio.github.io/thesis-tools/tools/ui-prototypes/`.** Verified: zero non-local requests with all artifacts loaded, Montserrat resolves locally, round-3 audit recomputes live (7.4:1 matches record), no horizontal scroll | `tools/ui-prototypes/index.html`, `tools/ui-prototypes/SPEC.md`, `tools/ui-prototypes/rounds/`, `assets/fonts/montserrat/`, `index.html`, `README.md`, `ROADMAP.md`, `CLAUDE.md`, `.planning/quick/20260901-ui-prototypes/` |
