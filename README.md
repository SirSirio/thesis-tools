...
| [Prototype Design Space](prototypes/index.html) | Visual animated journey through the hardware prototypes, now carrying **two module threads** — the pump module that meters the dose (orange) and the alignment module that indexes the samples under it (violet), each with its own curve, its own node set and its own scroll animation. Every built prototype has its own **subpage**, co-located with that prototype's notes and media, giving purpose, as-built parameters, worked calculations, measured results and design reasoning. Authored from real test data throughout: the 5 µL 4-roller baseline, its corrected-geometry successor (gravimetrically qualified at 4.56 µL/stroke), and the bench-validated rack-indexing stage with homing. | ✅ Live |
