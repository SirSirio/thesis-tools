...
│   ├── thesis-defense/         The 30 minute defense deck (28 Sept 2026), built 2026-09-20. index.html is GENERATED by assemble.py from parts/ (00-head, 10-opening, 20-part1, 30-part2, 40-part3, 50-discussion, 60-backups, 99-tail): edit a part, re-run the assembler, never edit index.html. SPEC.md (runtime, three views, sync protocol), BRIEF.md (section 0 is the handoff: state, how to run, what is next; then requirements and decisions), PUNCHLIST.md (review observations and revision notes), CONTENT.md (the 43 slides and 23 backups with motion notes), BUILD-CONTRACT.md and BUILDER-REFERENCE.md (how to write a part), ASSETS.md, SKILL-NOTES.md, research/ (thesis reports), assets/ (airport photo, rasterised figures, stills, ui captures, posters, QR), previews/ (the three dialect previews, design references), screens/ (verification screenshots). Views: ?view=stage|presenter|guest; sync ?sync=host|guest over ntfy.sh plus BroadcastChannel. it/ (Italian guest deck) not built yet. Uses the deck.js state API (window.Deck, deck:state) added 2026-09-20
│   └── <slug>/
