...
│   ├── deck.js             Shared deck runtime (state machine) — D-01 exception. Since 2026-09-20 also exposes window.Deck (slideNo, step, cue, goToState, goToCue, slide(cue, builder), enter(cue, fn)), the deck:state event, the #/<slide>/<step> hash, PageUp/PageDown/Home/End, b blackout, f fullscreen, digits then Enter; additive, older decks unchanged
...
│   │   └── video/          H.264 web clips, muted, each beside its -poster.jpg: pump-head, droplet-slowmo, alignment-v2, pump-gravimetric (module clips, portrait or square), plus the four whole-machine clips added 2026-09-20 from the thesis exports: machine-dispensing (7 s landscape), machine-dispensing-portrait (27 s), machine-demo (20 s sped-up run), machine-outdoor-run (44 s outdoor run on battery)
│   └── gsap/
│       ├── gsap.min.js     Vendored GSAP core 3.15, no CDN — landing-page hero motif sync (D-01/D-03 exception) and the defense deck
│       ├── DrawSVGPlugin.min.js     Vendored 2026-09-20 for the defense deck only: lines that draw themselves
│       ├── MotionPathPlugin.min.js  Vendored 2026-09-20 for the defense deck only: glyphs travelling a path
│       └── MorphSVGPlugin.min.js    Vendored 2026-09-20 for the defense deck only: shape morphs (GSAP plugins are free since 3.13)
├── decks/
│   ├── index.html              Presentations index — card deal-out magic
│   ├── thesis-defense/         The 30 minute defense deck (28 Sept 2026), built 2026-09-20. index.html is GENERATED by assemble.py from parts/ (00-head, 10-opening, 20-part1, 30-part2, 40-part3, 50-discussion, 60-backups, 99-tail): edit a part, re-run the assembler, never edit index.html. ASSISTANT-GUIDE.md (the standing working agreement for the per-part chats: role, delegation, reply rules, which part owns which slides and thesis chapters — note chapter 5 sits in thesis Part II but presentation Part I — editing limits, content rules, verification, traps), SPEC.md (runtime, three views, sync protocol), BRIEF.md (section 0 is the handoff: state, how to run, what is next; then requirements and decisions), SLIDE-ORDER.md (how the deck is sorted: the band/topic taxonomy the overview groups by, `data-part="backup"` as the one thing that makes a slide an appendix slide, `A01…` numbering in the appendix slate, and the recipes for adding a slide, moving one into the appendix or adding a topic — read it before moving or renumbering anything), PUNCHLIST.md (review observations and revision notes), CONTENT.md (the talk slides and the appendix with motion notes), BUILD-CONTRACT.md and BUILDER-REFERENCE.md (how to write a part), ASSETS.md, SKILL-NOTES.md, research/ (thesis reports, plus `qa-readout-methods.md` — defence Q&A prep on signal-readout modalities a modular reader could use, for the §13.6 "pair it with a reader" question; not slide content), assets/ (airport photo, rasterised figures, stills, ui captures, posters, QR), previews/ (the three dialect previews, design references), screens/ (verification screenshots). Views: ?view=stage|presenter|guest; sync ?sync=host|guest over ntfy.sh plus BroadcastChannel. it/ (Italian guest deck) not built yet. Uses the deck.js state API (window.Deck, deck:state) added 2026-09-20
│   └── <slug>/
...
│   └── ui-mockup/
│       ├── index.html      Live User Interface — interactive mock of the finished firmware interface at the address chapter 9 reserved (tools/ui-mockup/, path frozen). Device-captured frames served pixel-exact with per-screen hotspots (geometry from the screen builders, destinations from the firmware navigation table), a conditions panel for the interlock variants, timed run/fill/reset/calibration sequences, the guided demo, and a two-way linked screen map. No calculator (D-01). English-only
│       ├── SPEC.md         Tool spec — the address contract, frame index (30 screens, 134 frames), condition-to-variant rules, hotspot geometry sources, sequence timings, what is real, staged and faked
│       └── screens/        The 134 device captures at 1× (320 × 240 PNG), copied from the firmware documentation's device-shots set — DO NOT EDIT (dated thesis evidence; see SPEC.md)
├── prototypes/
...
- **Animations:** `fade-up` entrance with staggered `--delay`, hover lift + glow on cards. The landing page's hero motif additionally uses locally-vendored **GSAP core** (`assets/gsap/gsap.min.js`, no plugins, no CDN) to drive one loop clock for the hero pump (rebuilt 2026-09-23 from the defense deck's PumpAnim figure, re-implemented inline in plain SVG maths: liquid drawn from a reagent bottle through a dip tube, a 4-roller rotor turning stop-and-go under a 180° track with the tube pinched under every roller, the lit packet swept out one quarter turn at a time, one drop per stroke into a receiving tube, the bottle level falling in step; pauses off-screen, still frame under reduced motion) — a D-01/D-03 exception approved specifically for that motif; every other page's motion stays hand-built CSS/SVG.
...
- The sanctioned shared files, and only these, are:
  ...
  - `assets/gsap/`, vendored GSAP core (landing page hero motif, D-01/D-03 exception) plus the DrawSVG, MotionPath and MorphSVG plugins used only by the defense deck `decks/thesis-defense/`
